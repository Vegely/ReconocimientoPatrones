...
\begin{document}
	...
	\begin{figure}[H]
		...
		\caption{Arquitectura del Módulo C3-k (con CV de entrada y repetición)}
		\label{fig:c3_final}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption{Arquitectura SPPF en Cascada (Staircase Flow)}
	...
	\begin{figure}[H] % [p] fuerza una página dedicada si es necesario
		...
		% Ajustar al ancho de página
			...
			\begin{tikzpicture}[
				...
				% --- Nivel 2: Bajada P4 ---
				\node [resizeblock, below=1.0cm of cv_mid] (resize2) {Resize};
				\draw [line] (cv_mid) -- (resize2);
				...
				\draw [line] (resize2.south) -- (csp3.north) node[midway, right, font=\small,yshift=-20,xshift=-15] {1};
				...
				at={($(c3_2.south) + (5, 6.0)$)}] {
					\textbf{Leyenda:}\\
					 \textbf{CV}: Convolución (Conv2d+BN+SiLU)\\
					...
					\textbf{1, 2}: Orden de concatenación en el flujo
				};
			\end{tikzpicture}
	...
	\appendix
	\newpage
	\section{Trabajo realizado con IA}
	\subsection{Redes Convolucionales}
	...
	\subsection{Metodología Yolo}
	La metodología YOLO propone un enfoque de "disparo único" (\textit{single-shot}). A diferencia de sistemas R-CNN, YOLO divide la imagen térmica en una rejilla de $S \times S$. Si el centro de un objeto térmico cae en una celda, esa celda es responsable de su detección. Esto reduce drásticamente la latencia, permitiendo el monitoreo de incendios o personas en tiempo real.
	
	
	
	\subsection{Neurona de la red Yolo}
	Cada neurona en las capas de predicción genera un vector de salida que contiene la probabilidad de clase $P(C)$ y los parámetros de la caja delimitadora. Para imágenes térmicas, se ajusta el umbral de confianza (\textit{threshold}) para evitar falsos positivos producidos por fuentes de calor estáticas (como maquinaria o motores).
	
	\subsection{Topología de la red}
	La estructura se organiza en bloques funcionales que procesan la señal térmica:
	\begin{itemize}
		\item \textbf{Capa de Convolución:} Representada comúnmente como \texttt{cvblock} , extrae rasgos térmicos.
		\item \textbf{Max Pooling:} El bloque \texttt{mpblock} reduce la dimensionalidad.
		\item \textbf{ResNet Blocks:} Los \texttt{resnetblock} permiten entrenar redes profundas sin degradación del gradiente.
		\item \textbf{Concatenación:} Los \texttt{concatblock}  fusionan mapas de características de diferentes escalas.
	\end{itemize}
